...
const Berries = () => {
  ...
  const { data } = useQuery(["berries", offset], () =>
      ...
      <div className={styles.pageContainer}>
        <div className={styles.description}>
          <h4>
            ...
            effects. First found in the Generation II games, many Berries have
            since become critical held items in battle, where their various
            effects include HP and status condition restoration, stat
            enhancement, and even damage negation.
          </h4>
        </div>
        <div className={styles.grid}>
          {names &&
            names.map((name) => <BerriesDetails key={name} berryName={name} />)}
        </div>
        {data && (
